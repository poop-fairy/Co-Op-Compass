...
def call_url(url: str, params: Dict[str, str] = None) -> List[Dict]:
    ...
    """
    try:
        with requests.get(url, params=params if params else None) as response:
...
def build_xbox_gamepass_url(first: str, last: str, xbox_gamepass_ids: List[str]) -> str:
    ...
    """
    return f"{first}{','.join(xbox_gamepass_ids)}{last}"


def extract_xbox_ids(xbox_gamepass_network_data: List[Dict[str, str]]) -> List[str]:
    ...
    return [list(id_key.values())[0] for id_key in xbox_gamepass_network_data[1:]]
...
def search_game(game_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        rawg_response = call_url(RAWG_URL, params=params)
        return rawg_response.get("results", [])
...
def extract_game_data(game_data: List[Dict[str, Any]]) -> Tuple[str, str, List[str]]:
    """
    Extracts game release date, background image, and genres from game data.

    Args:
        game_data (List[Dict[str, Any]]): A list containing dictionaries with game information.

    Returns:
        Tuple[str, str, List[str]]: A tuple containing the release date, background image URL, and a list of genres.
    """
    # Check if game_data is not empty and has the necessary structure
    if not game_data or "tags" not in game_data[0]:
        return ("", "", [])

    # Check for co-op in tags
    if any("co-op" in str(tag) for tag in game_data[0]["tags"]):
        released_on = game_data[0]["released"]
        background_image = game_data[0]["background_image"]
        genres = [genre["name"] for genre in game_data[0].get("genres", [])]
        return (released_on, background_image, genres)

    return ("", "", [])  # Return empty values if no co-op tag is found
...
def main() -> None:
    ...
    6. Matches games to RAWG database and extracts release_data, genre, and image for co-op games.
    """
    # Fetch and clean PS5 Game Pass games
    ps_response_json = call_url(PS_GAMEPASS_URL)
    ...
    xbox_gamepass_network_data = call_url(XBOX_GAMEPASS_SHELL_URL)
    ...
    xbox_response_json = call_url(xbox_gamepass_url)
    ...
    coop_games: Dict[str, Tuple[str, str, str]] = {}

    for ps5_game, match, score in common_games:
        print(
            f"PS5 Game: '{ps5_game}' | Xbox Match: '{match}' | Similarity Score: {score}"
        )

        game_data = search_game(ps5_game)  # Matches games to RAWG database
        coop_games[ps5_game] = extract_game_data(
            game_data
        )  # Store release_data, genre, and image for co-op games

    pprint.pprint(coop_games)
